...
const STORAGE_KEY = "bk_asesmen_gaya_belajar"
      ...
      {states.map((s, i) => (
        <button
          ...
          className={`flex items-center justify-center rounded-full transition-all duration-200 ${
            s === "current" ? "h-7 w-7 ring-2 ring-blue-400 ring-offset-1 scale-110" :
            s === "answered" ? "h-6 w-6 bg-blue-500 text-white" :
        ...
        >
          {s === "answered" ? <Check className="h-3 w-3" /> :
           ...
           s === "current" ? <span className="h-2.5 w-2.5 rounded-full bg-blue-600" /> :
...
export function GayaBelajarForm() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    fetch(`/api/siswa/asesmen/status?nisn=${encodeURIComponent(user.username)}&jenis=gaya-belajar`)
      .then((r) => r.json())
  ...
  const checkStatus = useCallback(async () => {
    if (!nama || !kelas || !user?.username) return
    try {
      const r = await fetch(`/api/siswa/asesmen/status?nisn=${encodeURIComponent(user.username)}&jenis=gaya-belajar`)
  ...
  async function requestRetake() {
    ...
    try {
      const res = await fetch("/api/siswa/retake", {
        ...
        body: JSON.stringify({ nisn: user.username, jenis: "gaya-belajar" }),
      })
  ...
  function answer(dimensi: string) {
    setJawaban((prev) => ({ ...prev, [questions[step].id]: dimensi }))
