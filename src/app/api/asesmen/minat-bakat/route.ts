...
export async function POST(req: Request) {
  try {
    ...
    const result = await prisma.minatBakat.create({
      data: {
        siswaId: siswa.id,
    ...
    await prisma.retakeRequest.deleteMany({ where: { siswaId: siswa.id, jenis: "minat-bakat" } })
...
export async function GET(req: Request) {
  ...
  const data = await prisma.minatBakat.findMany({
    where,
    include: { siswa: true },
