...
export async function POST(req: Request) {
  try {
    ...
    const result = await prisma.gayaBelajar.create({
      data: {
        siswaId: siswa.id,
    ...
    await prisma.retakeRequest.deleteMany({ where: { siswaId: siswa.id, jenis: "gaya-belajar" } })
...
export async function GET(req: Request) {
  ...
  const data = await prisma.gayaBelajar.findMany({
    where,
    include: { siswa: true },
