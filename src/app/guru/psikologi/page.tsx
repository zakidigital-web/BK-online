...
import { ArrowLeft, ArrowRight, Check, GraduationCap, Brain, User, Heart, ClipboardList, Circle, CheckCircle2 } from "lucide-react"
      ...
      {states.map((s, i) => (
        <div
          ...
          className={`flex items-center justify-center rounded-full transition-all duration-200 ${
            s === "current" ? "h-7 w-7 ring-2 ring-rose-400 ring-offset-1 scale-110" :
            s === "answered" ? "h-6 w-6 bg-rose-500 text-white" :
            "h-5 w-5 border border-gray-300 text-gray-300"
          }`}
        >
          {s === "answered" ? <Check className="h-3 w-3" /> :
           s === "current" ? <span className="h-2.5 w-2.5 rounded-full bg-rose-600" /> :
...
export default function GuruPsikologiPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/guru/asesmen?guruId=${user.id}`)
      ...
      .then((data) => {
        if (data.asesmen?.skorPsikologi) setExisting(JSON.parse(data.asesmen.skorPsikologi))
      })
      .catch(() => {})
  }, [user])
  ...
  async function submit() {
    ...
    try {
      ...
      setHasil(skor)
      toast.success("Psikologi guru selesai!")
    } catch {
      toast.error("Gagal menyimpan psikologi")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (existing && !hasil && !showReview) {
    const status = getStatusPsikologi(existing)
    return (
      <div className="max-w-lg mx-auto space-y-4">
        <Card className="border-0 bg-gradient-to-br from-rose-50 to-pink-50 shadow-sm">
          <CardContent className="p-6 text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900">Psikologi Tersimpan</h2>
            <p className="mt-2 text-sm text-gray-500">Anda sudah pernah mengerjakan asesmen psikologi.</p>
            <Badge className="mt-3 bg-rose-500 text-white">{status.label}</Badge>
            <div className="mt-6 flex gap-3 justify-center">
              <Button onClick={() => router.push("/guru/laporan")} className="bg-rose-600 hover:bg-rose-700">
                Lihat Hasil
              </Button>
      ...
      <div className="space-y-6">
        <Card className="border-0 bg-gradient-to-br from-rose-50 to-pink-50 shadow-sm">
          <CardContent className="p-6 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900">Psikologi Guru</h2>
            <p className="mt-1 text-rose-600 font-medium">{user?.name}</p>
            <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-rose-500 px-4 py-1.5 text-sm font-semibold text-white">
              {status.label}
            </div>
            <p className="mt-3 text-sm text-gray-600 max-w-md mx-auto">{status.desc}</p>
          </CardContent>
        </Card>
        ...
        <div className="flex gap-3">
          ...
          <Button className="flex-1 bg-rose-600 hover:bg-rose-700 gap-2" onClick={() => router.push("/guru/laporan")}>
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="flex justify-between">
          ...
          <Button onClick={submit} disabled={submitting} className="bg-rose-600 hover:bg-rose-700 gap-2">
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <h1 className="text-lg font-bold text-gray-900">Psikologi Guru</h1>
            <span className="text-sm font-medium text-gray-400">{answered}/{totalSteps}</span>
          </div>
          {(() => {
            const dotStates: DotState[] = psikologiSoals.map((s, i) => {
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Card className="border-0 shadow-sm overflow-hidden">
            <div className="h-1.5" style={{ backgroundColor: dimensiWarna[q.dimensi] || "#e11d48" }} />
            <CardContent className="p-6">
              <div className="mb-3 flex items-center gap-2 text-xs font-medium uppercase tracking-wider"
                style={{ color: dimensiWarna[q.dimensi] || "#e11d48" }}>
                    ...
                    <button
                      ...
                      className={`group relative flex flex-col items-center justify-center rounded-xl border-2 p-3 transition-all duration-200 ${
                        isSelected
                          ...
                          : "border-gray-100 bg-gray-50 hover:border-rose-200 hover:bg-rose-50/50 hover:scale-[1.02]"
                      }`}
                    >
                      <span className={`text-xl font-bold transition-colors ${isSelected ? "text-rose-700" : "text-gray-400 group-hover:text-rose-600"}`}>
                        {val}
                      </span>
                      <span className={`mt-1 text-center text-[10px] leading-tight transition-colors ${isSelected ? "text-rose-700 font-medium" : "text-gray-400 group-hover:text-rose-600"}`}>
                        {skalaLabel[val - 1]}
                      </span>
                    </button>
                  )
      ...
      <div className="flex justify-between">
        ...
        {isDone ? (
          <Button onClick={() => setShowReview(true)} className="bg-rose-600 hover:bg-rose-700 gap-2">
